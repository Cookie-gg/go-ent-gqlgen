...
func (po *PostQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "message":
			if _, ok := fieldSeen[post.FieldMessage]; !ok {
				selectedFields = append(selectedFields, post.FieldMessage)
				fieldSeen[post.FieldMessage] = struct{}{}
			}
		case "createdAt":
		...
		case "likes":
			if _, ok := fieldSeen[post.FieldLikes]; !ok {
				selectedFields = append(selectedFields, post.FieldLikes)
				fieldSeen[post.FieldLikes] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
